...
type Props = {
  params: { orgUsername: string }
}

// Function to generate metadata dynamically based on the organization's data
export async function generateMetadata(
  { params }: Props,
  parent: ResolvingMetadata,
): Promise<Metadata> {
  const { orgUsername } = params
  const response = await fetch(
    `${defaultValues.backendURL}/api/v0/public/organizations/orgUsername-${orgUsername}`,
  )
  const organization = await response.json()

  const title = `${organization.data.name} - Organization Profile - Edust`
  const description = `View the profile of ${organization.data.name} on Edust. Explore detailed information about the organization, including location and profile picture.`
  const previousImages = (await parent).openGraph?.images || []

  return {
    title,
    description,
    keywords: `organization profile, Edust, ${organization.data.name}, educational organizations, location, profile picture`,
    authors: [{ name: "Edust" }],
    openGraph: {
      title,
      description,
      images: [
        organization.data.profilePic || "/default-image.jpg",
        ...previousImages,
      ],
      type: "website",
    },
